...
def ta1(flows, datetimes, hydro_years, drainage_area):
    ...
    colwell_y = np.sum(colwell, axis=-2)  # sum up values in each column (i.e. state)
...
def ta2(flows, datetimes, hydro_years, drainage_area):
    ...
    colwell_hx = \
        - np.nansum(np.multiply(colwell_xz, colwell_log_xz, dtype=np.float64), axis=-1)
    colwell_z = np.reshape(colwell_z, (colwell_z.shape[0], 1, 1))
    ...
    colwell_hxy = \
        - np.nansum(np.multiply(colwell_mz, colwell_log_mz, dtype=np.float64), axis=(-1, -2))
...
def th1(flows, datetimes, hydro_years, drainage_area):
    ...
    info = np.zeros((flows.shape[0], hydro_years.shape[0], 2), dtype=np.float64)
    for hy, mask in enumerate(hydro_years):
        max_julian_day = pd.DatetimeIndex(datetimes[mask][np.argmax(flows[:, mask], axis=-1)]).dayofyear
