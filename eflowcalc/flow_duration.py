...
def dl2(flows, datetimes, hydro_years, drainage_area):
    ...
    for hy, mask in enumerate(hydro_years):
        if hy == 0:  # i.e. first year in period
            info[:, hy] = np.amin(roll_3[:, 0:(np.sum(mask) - 1)], axis=-1)
            i += np.sum(mask) - 1
        elif hy == (np.sum(mask) - 1):  # i.e. last year in period
            info[:, hy] = np.amin(roll_3[:, i:(i + np.sum(mask) - 2)], axis=-1)
            i += np.sum(mask)
        else:
            info[:, hy] = np.amin(roll_3[:, i:(i + np.sum(mask))], axis=-1)
            i += np.sum(mask)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dl3(flows, datetimes, hydro_years, drainage_area):
    ...
    for hy, mask in enumerate(hydro_years):
        if hy == 0:  # i.e. first year in period
            info[:, hy] = np.amin(roll_7[:, 0:(np.sum(mask) - 3)], axis=-1)
            i += np.sum(mask) - 3
        elif hy == (np.sum(mask) - 1):  # i.e. last year in period
            info[:, hy] = np.amin(roll_7[:, i:(i + np.sum(mask) - 4)], axis=-1)
            i += np.sum(mask)
        else:
            info[:, hy] = np.amin(roll_7[:, i:(i + np.sum(mask))], axis=-1)
            i += np.sum(mask)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dl5(flows, datetimes, hydro_years, drainage_area):
    ...
    for hy, mask in enumerate(hydro_years):
        if hy == 0:  # i.e. first year in period
            info[:, hy] = np.amin(roll_90[:, 0:(np.sum(mask) - 44)], axis=-1)
            i += np.sum(mask) - 44
        elif hy == (np.sum(mask) - 1):  # i.e. last year in period
            info[:, hy] = np.amin(roll_90[:, i:(i + np.sum(mask) - 45)], axis=-1)
            i += np.sum(mask)
        else:
            info[:, hy] = np.amin(roll_90[:, i:(i + np.sum(mask))], axis=-1)
            i += np.sum(mask)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dl14(flows, datetimes, hydro_years, drainage_area):
    median = np.median(flows, axis=-1)
    perc25 = np.percentile(flows, 25, axis=-1)
...
def dl15(flows, datetimes, hydro_years, drainage_area):
    median = np.median(flows, axis=-1)
    perc10 = np.percentile(flows, 10, axis=-1)
...
def dl18(flows, datetimes, hydro_years, drainage_area):
    ...
    info = np.zeros((flows.shape[0], hydro_years.shape[0]), dtype=np.float64)
    for hy, mask in enumerate(hydro_years):
        info[:, hy] = np.sum(flows[:, mask] == 0, axis=-1)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dl20(flows, datetimes, hydro_years, drainage_area):
    ...
    zero_flow = np.array(pd.DataFrame(flows.T != 0, index=datetimes).groupby(lambda x: (x.year, x.month)).sum()).T
    # calculations for entire time series
    sfc = np.sum(zero_flow == 0, axis=-1)
...
def dh2(flows, datetimes, hydro_years, drainage_area):
    ...
    for hy, mask in enumerate(hydro_years):
        if hy == 0:  # i.e. first year in period
            info[:, hy] = np.amax(roll_3[:, 0:(np.sum(mask) - 1)], axis=-1)
            i += np.sum(mask) - 1
        elif hy == (np.sum(mask) - 1):  # i.e. last year in period
            info[:, hy] = np.amax(roll_3[:, i:(i + np.sum(mask) - 2)], axis=-1)
            i += np.sum(mask)
        else:
            info[:, hy] = np.amax(roll_3[:, i:(i + np.sum(mask))], axis=-1)
            i += np.sum(mask)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dh3(flows, datetimes, hydro_years, drainage_area):
    ...
    for hy, mask in enumerate(hydro_years):
        if hy == 0:  # i.e. first year in period
            info[:, hy] = np.amax(roll_7[:, 0:(np.sum(mask) - 3)], axis=-1)
            i += np.sum(mask) - 3
        elif hy == (np.sum(mask) - 1):  # i.e. last year in period
            info[:, hy] = np.amax(roll_7[:, i:(i + np.sum(mask) - 4)], axis=-1)
            i += np.sum(mask)
        else:
            info[:, hy] = np.amax(roll_7[:, i:(i + np.sum(mask))], axis=-1)
            i += np.sum(mask)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dh5(flows, datetimes, hydro_years, drainage_area):
    ...
    for hy, mask in enumerate(hydro_years):
        if hy == 0:  # i.e. first year in period
            info[:, hy] = np.amax(roll_90[:, 0:(np.sum(mask) - 44)], axis=-1)
            i += np.sum(mask) - 44
        elif hy == (np.sum(mask) - 1):  # i.e. last year in period
            info[:, hy] = np.amax(roll_90[:, i:(i + np.sum(mask) - 45)], axis=-1)
            i += np.sum(mask)
        else:
            info[:, hy] = np.amax(roll_90[:, i:(i + np.sum(mask))], axis=-1)
            i += np.sum(mask)
    # calculations for entire time series
    sfc = np.mean(info, axis=-1)
...
def dh14(flows, datetimes, hydro_years, drainage_area):
    # calculations per month for each year
    mean = np.array(pd.DataFrame(flows.T, index=datetimes).groupby(lambda x: (x.year, x.month)).mean()).T
    # calculations for entire time series
    perc95 = np.percentile(mean, 95, axis=-1)
    sfc = perc95 / np.mean(mean, axis=-1)
